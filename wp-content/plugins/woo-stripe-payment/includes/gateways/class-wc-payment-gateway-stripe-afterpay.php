	...
	protected function get_payment_description() {
		...
		if ( ( $country = stripe_wc()->account_settings->get_option( 'country' ) ) ) {
			...
			foreach ( $params as $currency => $param ) {
				if ( $param[0] === $country ) {
					$desc = sprintf( __( 'Store currency must be %s for Afterpay to show because your Stripe account is registered in %s. This is a requirement of Afterpay.', 'woo-stripe-payment' ), $currency, $country );
					break;
				}
			}
		}
